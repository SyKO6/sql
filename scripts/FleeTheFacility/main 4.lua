...
local function createESP(target)  
	...
	nameLabel.TextTransparency = 0.15 -- 🧿 75% visible
...
local TEMMIE_HEIGHT = TEMMIE_WIDTH * (2496 / 1920)  -- mantiene proporción
...
local function createTemmieBillboard(originalBill)  
	...
	imageLabel.ImageTransparency = 0.35 -- 🟦 Por defecto 90% visible (0 = opaco, 1 = invisible)  
			...
			if screen and hl then  
				...
				if temmie and temmie:IsA("BillboardGui") then  
					...
					if adornee and adornee:IsA("Attachment") and adornee.Parent then  
						...
						if image then  
							image.ImageTransparency = 1 - (data.alpha * 0.65) -- límite 0.1 mínima (90% visible máximo)  
						end  
