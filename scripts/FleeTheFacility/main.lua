    ...
    if list then
        for _, v in ipairs(list) do
            pcall(function()
                ...
                elseif typeof(v) == "Instance" and v.Destroy then v:Destroy() end
            end)
        end
    end
    playerCleanup[p] = nil
...
local function colorClose(a, b, tol)
    ...
    return math.abs(ar - br) <= tol and math.abs(ag - bg) <= tol and math.abs(ab - bb) <= tol
end
local DISABLED_COLOR = Color3.fromRGB(40,127,71)
            ...
            for _, child in ipairs(obj:GetChildren()) do
                if child.Name == "ComputerTable" and child:IsA("Model") then
                    ...
                    if screen then
                        ...
                        if originalBill then createTemmieBillboard(originalBill) end
                    end
                end
            end
    ...
    task.defer(function()
        ...
        elseif child:IsA("Model") then
            for _, c in ipairs(child:GetChildren()) do
                if c.Name == "ComputerTable" and c:IsA("Model") then
                    ensureTableContour(c)
                    local screen = c:FindFirstChild("Screen")
                    if screen then
                        local bill = screen:FindFirstChild("BillboardGui")
                        if bill then createTemmieBillboard(bill) end
                    end
...
local function destroyESPForTarget(target)
    ...
    if target and target.Parent then target:SetAttribute("ESPAdded", nil) end
end
...
local function createESP(target)
    ...
    local targetTorso = char:FindFirstChild("HumanoidRootPart")
    activeESPs[target] = {
        highlight = highlight,
        billboard = billboard,
        nameLabel = nameLabel,
...
local function updateAllVisualChances()
    ...
    for _, plr in ipairs(allPlayers) do
        local baseChance = 0
        if plr:FindFirstChild("SavedPlayerStatsModule") and plr.SavedPlayerStatsModule:FindFirstChild("BeastChance") then
            baseChance = plr.SavedPlayerStatsModule.BeastChance.Value or 0
        end
        local visualChance
...
renderConn = RunService.RenderStepped:Connect(function(dt)
    ...
    for target, data in pairs(activeESPs) do
        if not data or not data.target or not data.target.Parent then
            ...
            local targetTorso = data.targetTorso or (data.target.Character and data.target.Character:FindFirstChild("HumanoidRootPart"))
            ...
            if currentAnimValue == "Typing" and priority < 2 then
                finalColor = Color3.fromRGB(0,255,0); priority = 2
            end
            if capturedValue and priority < 3 then
                finalColor = Color3.fromRGB(150,220,255); priority = 3
            end
            ...
            if data.nameLabel and data.nameLabel.Parent then
                local baseChance = (data.target:FindFirstChild("SavedPlayerStatsModule")
                    and data.target.SavedPlayerStatsModule:FindFirstChild("BeastChance")
                    and data.target.SavedPlayerStatsModule.BeastChance.Value) or 0
                local playerCount = math.max(#Players:GetPlayers(), 1)
                local visualChance = visualChances[data.target] or ((baseChance == 0) and 0 or math.clamp(baseChance + ((100 - baseChance) / playerCount), 0, 100))
